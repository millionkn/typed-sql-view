import { exec, SqlViewTemplate, SqlAdapter, createResolver, hasOneOf, Column } from "./tools.js";
...
export class SqlExecutor {
	...
	private rawSelectAll<VT extends SqlViewTemplate<''>>(
		...
		const resolver = exec(() => {
			...
			return {
				createHolder: (getValue: () => string) => {
					return resolver.createHolder(getValue)
				},
				resolve: (str: string) => {
		...
		const viewResult = SqlView.buildSelectAll(view, { order: flag.order }, {
			createHolder: () => {
				let getValue = (): string => { throw new Error() }
				return {
					expr: resolver.createHolder(() => getValue()),
