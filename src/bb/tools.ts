...
export const iterateTemplate = (template: SqlViewTemplate<string>, cb: (column: Column<string>) => unknown): unknown => {
...
export class Column<T extends string, R = unknown, N extends boolean = boolean> {
	...
	expr(assert: T) {
		if (this[sym].assert !== assert) {
			throw new Error(`assert tag '${assert}',but saved is '${this[sym].assert}'`)
		}
